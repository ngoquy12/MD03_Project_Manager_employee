...
route.use(
  cors({
    origin: "http://localhost:5173", // Chỉ định nguồn gốc cho phép gửi cookie (trang React của bạn)
    credentials: true, // Bật chia sẻ cookie
  })
);
        ...
        bcrypt.compare(Password, user.Password, (err, isMatch) => {
          if (err) {
            ...
            if (!isMatch) {
              ...
              // Mã hóa thông tin trước khi lưu trên cookie
              const token = jwt.sign({ userId: user.userId }, "secret_key", {
                expiresIn: "1h",
              });

              // Lưu dữ liệu vào trong cookie
              res.cookie("cookie_key", token, {
                maxAge: 3600000,
                expires: true, // http và https cho phép truy cập vào
                path: "/",
              });
...
route.get("/get/check-login", (req, res) => {
  const token = req.cookies.cookie_key;
  // Kiểm tra xem token và session có tồn tại hay không
  if (token) {
    ...
    jwt.verify(token, "secret_key", (err, decoded) => {
      if (err) {
        res.status(401).json({
          status: 401,
          message: "Phiên của bạn đã hết. Vui lòng đăng nhập lại!",
        });
      } else {
